...
The dataset is based on the Fondeyt Postdoctoral Fellowship database between the years 2010 and 2016.
...
output_file('fondecyt_postdoc.html', title='Centro de Estudios ANIP')
...
# Read the Fondecyt postdoc data
...
plot = Figure(plot_width=700, x_axis_type='datetime', title='Proyectos Fondecyt Postdoc', x_range=(datetime.date(2009,9,1),datetime.date(2016,6,1)), tools=TOOLS)
...
p0 = Paragraph(text="")
p1 = Paragraph(text="Análisis descriptivo de los Proyectos Fondecyt Postdoc en el período 2010-2016.")
...
for s in data_nombre_unique:
    print("'"+s+"': eval('"+s.replace(" ","_").lower()+"'),")
